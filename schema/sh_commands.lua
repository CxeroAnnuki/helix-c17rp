...
-- global vortessense chat messages
do
	local COMMAND = {}
	COMMAND.arguments = ix.type.text
	...
	function COMMAND:OnRun(client, message)
		...
		if client:IsVortigaunt() then
			ix.chat.Send(client, "vortessense", message)
		else
			client:Notify("You need to be a Vortigaunt to do this.")
		end
	end

	ix.command.Add("Vortessense", COMMAND)
end